use celeriant_disk::files::{read_fixed_records_visit_const::read_fixed_records_visit_const, read_objects_absolute::{AbsoluteObjectPosition, read_objects_absolute}};
use celeriant_msg::{request::read_filters::ReadFilters, response::responses::ReadResponse};
use celeriant_wal::{compression_type::CompressionType, constants::METADATA_BATCH_SIZE_BYTES, wal::event_batch_item::EventBatchItem};
use celeriant_wire::version_aware_wire_format::{deserialize_event_batch_metadata_versioned, deserialize_event_batch_versioned};
...
impl ReadOperations for ReadOperationsWithDmaFiles {
    ...
    async fn get_write_operations_data_requirements(
        ...
        let remainder = file_len_metadata % rec_size;
        if remainder != 0 {
            // Partial write at end - auto-repair by truncating to aligned length
            // This can happen if process was killed mid-write before sync completed
            let aligned_len = (file_len_metadata / rec_size) * rec_size;
            
            if aligned_len == 0 {
                // No complete records exist, treat as initial state
                let metadata_buffer = vec![0u8; metadata_dma_file.alignment() as usize];
                let event_batch_buffer = vec![0u8; event_batches_dma_file.alignment() as usize];

                return Ok(WriteOperationsDataRequirements {
                    file_len_event_batch: 0,
                    file_len_metadata: 0,
                    metadata_buffer,
                    event_batch_buffer,
            ...
            // Ignore corrupt data at the end and continue with aligned length
    ...
    async fn read(
        ...
        let event_batches_bytes_set = read_objects_absolute(
