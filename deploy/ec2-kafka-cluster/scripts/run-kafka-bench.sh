#!/bin/bash
# Run kafka-bench (Rust) on all EC2 client nodes and collect results locally.
#
# 1-for-1 mirror of deploy/ec2-cluster/scripts/run-benchmark.sh but for Kafka.
# Uses the kafka-bench binary which mirrors Celeriant's rpi_cluster_pool_bench:
# N concurrent tokio tasks, each doing synchronous produce-wait-for-ack.
#
...
#   BENCH_RECORD_SIZE — payload size in bytes (default: 256)
...
# Determine bootstrap and TLS settings
...
RESULT_FILE="$RESULT_DIR/${TIMESTAMP}_kafka-bench_${SAFE_TYPE}_${STORAGE_TYPE}.txt"
mkdir -p "$RESULT_DIR"
...
echo "  acks=all, batch.num.messages=1, linger.ms=0"
echo "  Output:      $RESULT_FILE"
echo ""

# Write metadata header to result file
cat > "$RESULT_FILE" <<EOF
# Kafka EC2 Benchmark (kafka-bench, mirrors rpi_cluster_pool_bench)
# Date:         $(date -Iseconds)
...
# acks:         all
# batching:     disabled (batch.num.messages=1, linger.ms=0)
...
echo "==> Running kafka-bench on $CLIENT_COUNT client(s)"
