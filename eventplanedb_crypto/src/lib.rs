...
use rsa::{RsaPublicKey, RsaPrivateKey, pkcs1v15::Signature, pkcs8::{DecodePublicKey, EncodePublicKey, DecodePrivateKey, EncodePrivateKey}, signature::{SignatureEncoding, Verifier, SignerMut}};
...
impl Crypto {
    ...
    pub fn sign_nonce(private_key_base64: &str, nonce: &str) -> Result<String, CryptoError> {
        ...
        let private_key_bytes = general_purpose::STANDARD.decode(private_key_base64)
    ...
    pub fn validate_signature(public_key: &str, nonce: &str, signature: &str) -> Result<(), CryptoError> {
        ...
        let signature_data = general_purpose::STANDARD.decode(signature)
            .map_err(|_| CryptoError::InvalidSignature)?;
        ...
        verifying_key.verify(nonce_data, &sig)
            .map_err(|_| CryptoError::InvalidSignature)?;
    ...
    pub fn validate_nonce(nonce: &str) -> Result<(), CryptoError> {
        ...
        let nonce_timestamp = nonce.parse::<u64>()
        ...
        // Check if nonce is from the future
        if nonce_timestamp > now {
            return Err(CryptoError::InvalidNonce);
        }

        // Calculate time difference in minutes
        let time_diff_ms = now.saturating_sub(nonce_timestamp);
        let time_diff_minutes = time_diff_ms as f64 / 60.0 / 1000.0;
