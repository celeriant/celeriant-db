...
pub trait CeleriantPoolApi: Send + Sync {
    ...
    fn write(&self, request: WriteRequest) -> impl Future<Output = Result<SuccessResponse, ClientError>> + Send;
    fn delete(&self, request: DeleteRequest) -> impl Future<Output = Result<SuccessResponse, ClientError>> + Send;
    fn trim_start(&self, request: TrimStartRequest) -> impl Future<Output = Result<SuccessResponse, ClientError>> + Send;
