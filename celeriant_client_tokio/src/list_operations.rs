...
//     max_shard: Option<u64>,
//     next_shard_to_try: u64,
//     seen: HashSet<AggregateKey>,
//     /// Track aggregates marked as deleted - needed because older WAL entries
//     /// won't have is_deleted=true even if a newer entry soft-deleted them
...
//     pub async fn next(&mut self) -> Option<Result<AggregateListItem, ClientError>> {
//         loop {
//             while let Some(item) = self.buffer.pop_front() {
//                 let key = AggregateKey::new(item.org_id, item.aggregate_type_id, item.aggregate_id);
                
//                 // Track deleted status - if we've seen this aggregate as deleted
//                 // (from a newer WAL entry), remember it
//                 if !self.seen.contains(&key) && item.is_deleted {
//                     self.deleted.insert(key.clone());
//                 }
                
//                 // Deduplication - skip if we've already processed this aggregate
//                 if !self.seen.insert(key.clone()) {
//                     continue;
//                 }
                
//                 // Determine actual deleted status: either this entry says deleted,
//                 // or we previously saw a deleted entry for this aggregate
//                 let is_deleted = item.is_deleted || self.deleted.contains(&key);
                
//                 // Apply filter - skip deleted unless include_deleted is set
//                 if !self.include_deleted && is_deleted {
//                     continue;
//                 }
                
//                 // Return with the correct deleted status
//                 return Some(Ok(AggregateListItem {
//                     org_id: item.org_id,
...
//     pub async fn collect(mut self) -> Result<Vec<AggregateListItem>, ClientError> {
//         let mut results = Vec::new();
//         while let Some(item) = self.next().await {
//             results.push(item?);
//         }
