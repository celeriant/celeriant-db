...
async fn dispatch_test(name: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match name {
        ...
        "batch_replicated_cleartext" => batch_replicated_cleartext::run().await,
        ...
        "invariant_protocol_version_pinned_per_connection" => invariant_protocol_version::pinned_per_connection().await,
        "invariant_watch_trim_floor_monotonic" => invariant_watch_trim_floor::monotonic().await,
        "invariant_schema_register_redirects_from_non_leader" => invariant_schema_redirect::redirects_from_non_leader().await,
        "invariant_list_iterators_terminate_at_shard_boundary" => invariant_list_termination::terminate_at_shard_boundary().await,
        "invariant_size_caps_enforced_end_to_end" => invariant_size_caps::caps_enforced_end_to_end().await,
        "invariant_pool_seed_only_routes_to_first_seed" => invariant_pool_seed_only_routing::seed_only_pool_routes_to_first_seed().await,
        ...
        "idempotency_cold_reconstruction" => idempotency_cold_reconstruction::run().await,
        "idempotency_negative_scan_load" => idempotency_negative_scan_load::run().await,
        "idempotency_negative_scan_present_client" => idempotency_negative_scan_present_client::run().await,
        "idempotency_restart_fanin" => idempotency_restart_fanin::run().await,
        "idempotency_trim_client_state" => idempotency_trim_client_state::run().await,
        "idempotency_delete_recreate" => idempotency_delete_recreate::run().await,
        "idempotency_across_seal" => idempotency_across_seal::run().await,
        "crash_kill9_under_load_durability" => crash_kill9_under_load_durability::run().await,
        "schema_sealed_segment_survives_crash" => schema_sealed_segment_survives_crash::run().await,
        "recovery_multiseg_read_amplification" => recovery_multiseg_read_amplification::run().await,
        "sidecar_size_tracks_cardinality" => sidecar_size_tracks_cardinality::run().await,
