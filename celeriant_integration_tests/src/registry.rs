...
pub fn all_tests() -> &'static [TestEntry] {
    ...
    &[
        ...
        TestEntry {
            name: "invariant_protocol_version_pinned_per_connection",
            description: "Wire invariant: protocol version is pinned by the first message, mid-connection V2->V3 switch is rejected",
            estimated_secs: 10,
            categories: &[Invariant],
            distributed: false,
        },
        TestEntry {
            name: "invariant_list_iterators_terminate_at_shard_boundary",
            description: "List invariant: open-ended shard discovery terminates with complete sets, even from start_shard u64::MAX, and an out-of-range shard errors rather than returning an empty page",
            estimated_secs: 15,
            categories: &[Invariant],
            distributed: false,
        },
        TestEntry {
            name: "invariant_schema_register_redirects_from_non_leader",
            description: "Client-routing invariant: schema registration on a follower follows the 2027 leader hint instead of failing with a schema error",
            estimated_secs: 30,
            categories: &[Invariant, Schema, Replication],
            distributed: true,
        },
        TestEntry {
            name: "invariant_watch_trim_floor_monotonic",
            description: "Watch invariant: two trims coalesced into one fsync window notify watchers with the highest floor, not the first",
            estimated_secs: 15,
            categories: &[Invariant],
            distributed: false,
        },
        TestEntry {
            name: "invariant_size_caps_enforced_end_to_end",
            description: "Wire invariant: an oversize response surfaces as MessageTooLarge (not all-nodes-unreachable), and the server's response cap holds for compressible bodies",
            estimated_secs: 10,
            categories: &[Invariant],
            distributed: false,
        },
        TestEntry {
            name: "invariant_pool_seed_only_routes_to_first_seed",
            description: "Pool-routing invariant: a seed-only pool (no primary) routes leader operations to the first seed instead of dialing an empty address",
            estimated_secs: 5,
            categories: &[Invariant],
            distributed: false,
        },
        TestEntry {
            name: "invariant_watch_dial_survives_bad_candidates",
            description: "Watch invariant: a garbage-speaking or silent candidate cannot abort or wedge watch establishment when a healthy seed is configured",
            estimated_secs: 20,
            categories: &[Invariant],
            distributed: false,
        },
        TestEntry {
            name: "invariant_watch_options_enforced",
            description: "Watch invariant: an overflowing max_shard_hint errors instead of a silently empty watch, and the pool's response cap reaches watch connections",
            estimated_secs: 10,
            categories: &[Invariant],
            distributed: false,
        },
        TestEntry {
            name: "invariant_metrics_exported_families_described",
            description: "Metrics invariant: every exported celeriant_ metric family carries a # HELP description on the live /metrics endpoint",
            estimated_secs: 10,
            categories: &[Invariant],
            distributed: false,
        },
