...
use crate::{count_events, is_leader, poll_converged_count, s3_cluster_config, write_event, MinioContainer, TestServer, FOLLOWER_CONVERGENCE_TIMEOUT};
...
pub async fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let follower_count_s1 =
        poll_converged_count(&mut follower_client, &key_shard_1, 3, FOLLOWER_CONVERGENCE_TIMEOUT).await?;
    let follower_count_s2 =
        poll_converged_count(&mut follower_client, &key_shard_2, 3, FOLLOWER_CONVERGENCE_TIMEOUT).await?;
