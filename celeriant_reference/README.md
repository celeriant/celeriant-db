...
- **Exactly-once writes** `client_event_index` derived from catch-up + `enforce_client_idempotency` on the server
- **OCC retry loops** re-derive state on conflict, retry with fresh `expected_event_batch_index`
