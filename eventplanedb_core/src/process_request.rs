...
impl ProcessRequest {
    ...
    async fn handle_read_all(
        ...
        if !read_result.uncached_metadata_set.is_empty() {
            ...
            reader.as_mut().unwrap().update_metadata_cache(read_result.uncached_metadata_set);
        }
    ...
    async fn handle_write_batches(
        ...
        writer.as_mut().unwrap().prepend_batches(&request.batches).await?;
    ...
    async fn handle_trim_start(
        ...
        let file_positions = r_reader.get_file_positions(
            r_writer.minimum_available_event_batch_index, request.keep_from_event_batch_index, r_writer.file_len_metadata, r_writer.file_len_event_batch).await?;

        r_writer.trim_start(file_positions.metadata_position, file_positions.event_batch_position).await?;

        let metadata_dma_file = r_writer.metadata_dma_file.dup().map_err(|_e| EventPlaneDBError::io_error())?;
        let event_batches_dma_file = r_writer.metadata_dma_file.dup().map_err(|_e| EventPlaneDBError::io_error())?;
    ...
    async fn handle_delete(
        ...
        // Delete files
        let data_root_folder = Path::new(&self.data_root_folder).to_path_buf();
        let metadata_path = data_root_folder.join(format!("{}/{}/{}/metadata.bin", request.org_id, request.aggregate_type_id, request.aggregate_id));
        let events_path = data_root_folder.join(format!("{}/{}/{}/event_batches.bin", request.org_id, request.aggregate_type_id, request.aggregate_id));
