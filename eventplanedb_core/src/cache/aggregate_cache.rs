...
pub struct AggregateCache {
    ...
    aggregates_lease: Rc<RefCell<HashMap<AggregateKey, Rc<AggregateLease>>>>, // This node can write to the aggregate until epoch expiry time
...
impl AggregateCache {
    ...
    pub fn get_aggregate_lease(&self, aggregate_key: &AggregateKey) -> Rc<AggregateLease> {
        ...
        let aggregate_lease = Rc::new(AggregateLease::new(aggregate_key.clone(), self.node_config.clone()));
        cache.insert(aggregate_key.clone(), aggregate_lease.clone());
        aggregate_lease.clone()
    }
