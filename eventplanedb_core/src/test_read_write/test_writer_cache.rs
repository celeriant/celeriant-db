    ...
    fn test_cache_hit_with_various_filters() {
        let handle = LocalExecutorBuilder::new(Placement::Fixed(0))
            .spawn(|| async move {
                ...
                let filters = ReadFilters::new(1).min_event_timestamp(2000).max_event_timestamp(4000);
        ...
        let handle = LocalExecutorBuilder::new(Placement::Fixed(0))
            .spawn(|| async move {
                ...
                // Manually clear the cache
                {
                    ...
                    writer_ref.data_cache.clear();
                    writer_ref.total_cache_size_bytes = 0; // Reset cache size tracking
            ...
            .spawn(|| async move {
                ...
                for i in 1..=20 {
                    ...
                    let append_options = WriteOptions {
                        ...
                        server_timestamp_millis: 1000 * i,
                        user_id: None,
                    };
                ...
                assert!(writer_ref.data_cache.len() < 20);
                ...
                let first_cached_index = writer_ref
                    .data_cache
                    .front()
                    .unwrap()
                    .event_batch_item
        ...
        let handle = LocalExecutorBuilder::new(Placement::Fixed(0))
            .spawn(|| async move {
...

                {
                    ...
                    let next_event_index_before = writer_ref.next_event_index;
                    let next_event_batch_index_before = writer_ref.next_event_batch_index;
                    let client_event_index_before = writer_ref
                    ...
                    assert_ne!(writer_ref.next_event_index, next_event_index_before);
                    assert_ne!(
                        writer_ref.next_event_batch_index,
                        next_event_batch_index_before
                    );
                    ...
                    assert_eq!(writer_ref.next_event_index, next_event_index_before);
                    assert_eq!(
                        writer_ref.next_event_batch_index,
                        next_event_batch_index_before
                    );
                    assert_eq!(
                        writer_ref.client_event_indexes.get(&100).copied().unwrap(),
        ...
        let handle = LocalExecutorBuilder::new(Placement::Fixed(0))
            .spawn(|| async move {
                ...
                // Verify all batches are cached
                {
                    let writer = aggregate_resources.get_writer(true).await.unwrap();
                    let writer_ref = writer.as_ref().unwrap();
                    assert_eq!(writer_ref.data_cache.len(), 10);
                ...
                // Dynamically reduce cache size
                {
                    let mut writer = aggregate_resources.get_writer_mut(true).await.unwrap();
                    let writer_ref = writer.as_mut().unwrap();
                    writer_ref.update_max_data_cache_size_bytes(3000); // Much smaller
                }
                ...
                // Verify cache was trimmed
                {
                    ...
                    assert!(writer_ref.data_cache.len() < 10);
