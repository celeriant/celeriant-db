...
namespace OpenAI_API
{
	...
	public abstract class EndpointBase
	{
		...
		private async Task<HttpResponseMessage> HttpRequestRaw(string url = null, HttpMethod verb = null, string jsonContent = null, bool streaming = false)
		{
			...
			else
			{
				...
				else
				{
					...
					var parsedError = System.Text.Json.JsonSerializer.Deserialize<ApiErrorResponse>(resultAsString, SourceGenerationContext.Default.ApiErrorResponse);
					try
					{
		...
		/// <exception cref="HttpRequestException">Throws an exception if a non-success HTTP response was returned</exception>
		internal async Task<string> HttpGetContent(string url = null, HttpMethod verb = null, string? jsonContent = null)
		{
			var response = await HttpRequestRaw(url, verb, jsonContent);
		...
		/// <exception cref="HttpRequestException">Throws an exception if a non-success HTTP response was returned</exception>
		internal async Task<Stream> HttpRequest(string url = null, HttpMethod verb = null, string? jsonContent = null)
		{
			var response = await HttpRequestRaw(url, verb, jsonContent);
		...
		private async Task<T> HttpRequest<T>(string url = null, HttpMethod verb = null, string? jsonContent = null) where T : ApiResultBase
		{
			var response = await HttpRequestRaw(url, verb, jsonContent);
			string resultAsString = await response.Content.ReadAsStringAsync();

			var res = System.Text.Json.JsonSerializer.Deserialize<T>(resultAsString);
			try
			{
        ...
        private async Task<ChatResult> HttpRequestChatResult(string url = null, HttpMethod verb = null, string? jsonContent = null)
        {
            var response = await HttpRequestRaw(url, verb, jsonContent);
            string resultAsString = await response.Content.ReadAsStringAsync();

            var res = System.Text.Json.JsonSerializer.Deserialize(resultAsString, SourceGenerationContext.Default.ChatResult);
            try
            {
                res.Organization = response.Headers.GetValues("Openai-Organization").FirstOrDefault();
                res.RequestId = response.Headers.GetValues("X-Request-ID").FirstOrDefault();
		...
		*/
		...
		/// <returns>An awaitable Task with the parsed result of type <typeparamref name="T"/></returns>
		/// <exception cref="HttpRequestException">Throws an exception if a non-success HTTP response was returned or if the result couldn't be parsed.</exception>
		internal async Task<T> HttpPost<T>(string url = null, string? jsonContent = null) where T : ApiResultBase
		{
			return await HttpRequest<T>(url, HttpMethod.Post, jsonContent);
		}

        internal async Task<ChatResult> HttpPostChatResult(string url = null, string? jsonContent = null)
        {
            return await HttpRequestChatResult(url, HttpMethod.Post, jsonContent);
        }
        ...
        /// <returns>An awaitable Task with the parsed result of type <typeparamref name="T"/></returns>
        /// <exception cref="HttpRequestException">Throws an exception if a non-success HTTP response was returned or if the result couldn't be parsed.</exception>
        internal async Task<T> HttpDelete<T>(string url = null, string? jsonContent = null) where T : ApiResultBase
		{
			return await HttpRequest<T>(url, HttpMethod.Delete, jsonContent);
		}
		...
		/// <returns>An awaitable Task with the parsed result of type <typeparamref name="T"/></returns>
		/// <exception cref="HttpRequestException">Throws an exception if a non-success HTTP response was returned or if the result couldn't be parsed.</exception>
		internal async Task<T> HttpPut<T>(string url = null, string? jsonContent = null) where T : ApiResultBase
		{
			return await HttpRequest<T>(url, HttpMethod.Put, jsonContent);
		}
		...
		*/
		...
		/// <returns>The HttpResponseMessage of the response, which is confirmed to be successful.</returns>
		/// <exception cref="HttpRequestException">Throws an exception if a non-success HTTP response was returned</exception>
		protected async IAsyncEnumerable<T> HttpStreamingRequest<T>(string url = null, HttpMethod verb = null, string? jsonContent = null) where T : ApiResultBase
		{
			var response = await HttpRequestRaw(url, verb, jsonContent, true);
			...
			using (StreamReader reader = new StreamReader(stream))
			{
				...
				while ((line = await reader.ReadLineAsync()) != null)
				{
					...
					{
						var res = System.Text.Json.JsonSerializer.Deserialize<T>(line);
