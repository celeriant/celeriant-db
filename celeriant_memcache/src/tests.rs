...
mod tests {
    ...
    use celeriant_wal::{
        aggregate_client_key::AggregateClientKey, aggregate_key::AggregateKey, constants::FIXED_BLOCK_SIZE_BYTES, datablocks::{
            datablock::Datablock,
    ...
    fn client_load_status_returns_false_when_not_loaded() {
        let mut cache = new_cache();
        let key = make_aggregate_key(1, 1, 1);
        let client_key = AggregateClientKey::new(key.clone(), 100);

        let (is_loaded, index) = cache.aggregate_client_load_status(&key, &client_key);
    ...
    fn client_load_status_returns_none_for_zero_sentinel() {
        ...
        cache.put_aggregate_client_into_cache(client_key.clone(), 0, false);
    ...
    fn client_load_status_prefers_queue_over_cache() {
        ...
        cache.put_aggregate_client_into_cache(client_key.clone(), 10, false);
    ...
    fn client_load_status_different_clients_independent() {
        let mut cache = new_cache();
        let key = make_aggregate_key(1, 1, 1);
        let client_key_100 = AggregateClientKey::new(key.clone(), 100);
        let client_key_200 = AggregateClientKey::new(key.clone(), 200);

        cache.add_to_pending_append_queue(&key, 5, 3, 100, 42, make_queue_item(None));

        // Client 100 is loaded
        let (is_loaded, index) = cache.aggregate_client_load_status(&key, &client_key_100);
        assert!(is_loaded);
        assert_eq!(index, Some(42));

        // Client 200 is not loaded
        let (is_loaded, index) = cache.aggregate_client_load_status(&key, &client_key_200);
        assert!(!is_loaded);
        assert!(index.is_none());
    }
    ...
    fn client_load_status_different_aggregates_independent() {
        ...
        let key1 = make_aggregate_key(1, 1, 1);
        let key2 = make_aggregate_key(1, 1, 2);
        let client_key_1 = AggregateClientKey::new(key1.clone(), 100);
        let client_key_2 = AggregateClientKey::new(key2.clone(), 100);

        cache.add_to_pending_append_queue(&key1, 5, 3, 100, 42, make_queue_item(None));

        // Client 100 on key1 is loaded
        let (is_loaded, index) = cache.aggregate_client_load_status(&key1, &client_key_1);
        assert!(is_loaded);
        assert_eq!(index, Some(42));

        // Client 100 on key2 is not loaded
        let (is_loaded, index) = cache.aggregate_client_load_status(&key2, &client_key_2);
        assert!(!is_loaded);
        assert!(index.is_none());
    }
    ...
    fn client_load_status_after_rollback_falls_back_to_cache() {
        let mut cache = new_cache();
        let key = make_aggregate_key(1, 1, 1);
        let client_key_1 = AggregateClientKey::new(key.clone(), 100);
        ...
        let (is_loaded, index) = cache.aggregate_client_load_status(&key, &client_key_1);
        assert!(is_loaded);
    ...
    fn aggregate_load_status_returns_exists_false_for_zero_batch_index() {
        ...
        cache.put_aggregate_into_cache(
            key.clone(),
            crate::mem_snapshot_aggregate::MemSnapshotAggregate::not_found(),
            0,
            0,
            false,
        );
    ...
    fn load_status_pattern_prevents_redundant_disk_loads() {
        ...
        let (is_loaded, _) = cache.aggregate_client_load_status(&key, &client_key);
        assert!(!is_loaded);

        // Simulate loading from disk and caching
        cache.put_aggregate_client_into_cache(client_key.clone(), 42, false);
