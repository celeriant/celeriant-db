...
pub fn handle_disable_user_job(
    ...
    if for_client_id.is_none() && for_user_id.is_none() {
        return Err(JobError::NotFound(format!("Unable to disable user access. At least a client id or a user id must be specified")));
    }
    ...
    let event_batch = user_access_cache.update_access_for_user(
            ...
            &context.current_client_id,
            context.current_user_id.as_deref(),
            &for_client_id.unwrap_or(0), //Edge case where user wants to disable user instead of client
