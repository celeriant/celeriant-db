...
fn create_events(count: usize, size: usize, base_index: u64) -> Vec<DatablockAggregateEvent> {
    (0..count)
        .map(|i| DatablockAggregateEvent {
            ...
            event_seq: 0, // Server assigns this
            event_id: None,
                    ...
                    let handle = LocalExecutorBuilder::new(Placement::Fixed(0))
                        .spawn(move || async move {
                            ...
                            for i in 0..iters {
                                ...
                                debug_assert!(result.min_aggregate_version > 0 || result.min_aggregate_version > 0);
                                black_box(result);
                            }
                    ...
                    let handle = LocalExecutorBuilder::new(Placement::Fixed(0))
                        .spawn(move || async move {
                            ...
                            for i in 0..iters {
                                ...
                                // Either way the WAL lookup ran, which is what this measures.
                                debug_assert!(
