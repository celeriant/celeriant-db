...
pub async fn write_message<T>(stream: &mut impl AsyncWriteExt, message: &T) -> Result<(), ProtocolError>
where
    T: Encode,
{
...
pub async fn read_message<T>(stream: &mut impl AsyncReadExt) -> Result<T, ProtocolError>
where
    T: Decode,
{
    // Read version
    let mut version_buf = [0u8; 1];
    if stream.read_exact(&mut version_buf).await? == 0 {
    ...
    if stream.read_exact(&mut length_buf).await? == 0 {
        return Err(ProtocolError::ConnectionClosed);
    }
