...
pub async fn read_request<R>(reader: &mut R) -> Result<(Request, bool), WireError>
where
...
/// Read a response from the wire protocol
pub async fn read_response<R>(reader: &mut R) -> Result<(Response, bool), WireError>
where
    R: AsyncReadExt + Unpin,
{
    // Read version (4 bytes)
