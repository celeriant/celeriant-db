...
impl AggregateToUserAccessLevel {
    ...
    pub fn update_cache_for_user(&mut self, client_id: &u128, user_id: Option<&str>, proposed_access_level: AccessLevel, allow_override_existing: bool) {
    ...
    pub fn get_access_level(&self, client_id: &u128, user_id: Option<&str>) -> AccessLevel {
        match user_id {
            Some(user_id) => {  
                self.get_access_level_for_user(user_id)
            },
            None => {
...
mod tests {
    ...
    fn test_update_cache_no_op_no_access_to_no_access() {
        ...
        cache.update_cache_for_user(&client_id, user_hash, AccessLevel::None, false);
    ...
    fn test_update_cache_grant_access_to_new_user() {
        ...
        cache.update_cache_for_user(&client_id, user_hash, AccessLevel::Viewer, false);
    ...
    fn test_update_cache_no_op_has_access_cannot_remove() {
        ...
        // First grant access
        cache.update_cache_for_user(&client_id, user_hash, AccessLevel::Contributor, false);
    ...
    fn test_update_cache_remove_access_with_override() {
        ...
        // First grant access
        cache.update_cache_for_user(&client_id, user_hash, AccessLevel::Owner, false);
        
        // Remove access with override allowed
        cache.update_cache_for_user(&client_id, user_hash, AccessLevel::None, true);
    ...
    fn test_update_cache_decrease_access_level_with_override() {
        ...
        cache.update_cache_for_user(&client_id, user_hash, AccessLevel::Viewer, true);
    ...
    fn test_update_cache_same_access_level_with_override() {
        ...
        cache.update_cache_for_user(&client_id, user_hash, AccessLevel::Contributor, true);
    ...
    fn test_multiple_users() {
        ...
        cache.update_cache_for_user(&111, Some("user1"), AccessLevel::Owner, false);
        cache.update_cache_for_user(&222, Some("user2"), AccessLevel::Contributor, false);
        cache.update_cache_for_user(&333, Some("user3"), AccessLevel::Viewer, false);
    ...
    fn test_count_reflects_additions_and_removals() {
        ...
        cache.update_cache_for_user(&111, Some("user1"), AccessLevel::Viewer, false);
        assert_eq!(cache.count(), 1);
        
        cache.update_cache_for_user(&222, Some("user2"), AccessLevel::Owner, false);
        assert_eq!(cache.count(), 2);
        
        cache.update_cache_for_user(&111, Some("user1"), AccessLevel::None, true);
        assert_eq!(cache.count(), 1);
        
        cache.update_cache_for_user(&222, Some("user2"), AccessLevel::None, true);
        assert_eq!(cache.count(), 0);
    }
    ...
    fn test_client_pki_access_basic() {
        ...
        cache.update_cache_for_user(&client_id, None, AccessLevel::Viewer, false);
        
        assert_eq!(cache.count(), 1);
        assert_eq!(cache.get_access_level_for_client(&client_id), AccessLevel::Viewer);
        assert_eq!(cache.get_access_level(&client_id, None), AccessLevel::Viewer);
    }
    ...
    fn test_oauth_overrides_pki_client_access() {
        ...
        cache.update_cache_for_user(&client_id, None, AccessLevel::Contributor, false);
        assert_eq!(cache.count(), 1);
        ...
        cache.update_cache_for_user(&client_id, Some(user_id), AccessLevel::Owner, false);
        ...
        assert_eq!(cache.get_access_level(&client_id, Some(user_id)), AccessLevel::Owner);
    }
    ...
    fn test_oauth_override_removes_even_with_no_new_access() {
        ...
        cache.update_cache_for_user(&client_id, Some(user_id), AccessLevel::None, false);
    ...
    fn test_multiple_clients_pki_access() {
        ...
        cache.update_cache_for_user(&client1, None, AccessLevel::Owner, false);
        cache.update_cache_for_user(&client2, None, AccessLevel::Contributor, false);
        cache.update_cache_for_user(&client3, None, AccessLevel::Viewer, false);
    ...
    fn test_mixed_pki_and_oauth_users() {
        ...
        // PKI clients
        cache.update_cache_for_user(&client1, None, AccessLevel::Viewer, false);
        cache.update_cache_for_user(&client2, None, AccessLevel::Contributor, false);
        
        // OAuth users (different client IDs, but shouldn't matter for user access)
        cache.update_cache_for_user(&33333, Some(user1), AccessLevel::Owner, false);
        cache.update_cache_for_user(&44444, Some(user2), AccessLevel::Contributor, false);
        
        assert_eq!(cache.count(), 4);
        
        // PKI access
        assert_eq!(cache.get_access_level(&client1, None), AccessLevel::Viewer);
        assert_eq!(cache.get_access_level(&client2, None), AccessLevel::Contributor);
        
        // OAuth access
        assert_eq!(cache.get_access_level(&33333, Some(user1)), AccessLevel::Owner);
        assert_eq!(cache.get_access_level(&44444, Some(user2)), AccessLevel::Contributor);
    }
    ...
    fn test_pki_client_upgrade_and_downgrade() {
        ...
        cache.update_cache_for_user(&client_id, None, AccessLevel::Viewer, true);
        assert_eq!(cache.get_access_level_for_client(&client_id), AccessLevel::Viewer);
    }
    ...
    fn test_same_user_different_clients_oauth_consolidation() {
        ...
        cache.update_cache_for_user(&client1, Some(user_id), AccessLevel::Viewer, false);
        assert_eq!(cache.count(), 1);
        ...
        cache.update_cache_for_user(&client2, Some(user_id), AccessLevel::Owner, false);
        assert_eq!(cache.count(), 1); // Still just one user entry
        assert_eq!(cache.get_access_level_for_user(user_id), AccessLevel::Owner);
        
        // Access from either client should return the same user access level
        assert_eq!(cache.get_access_level(&client1, Some(user_id)), AccessLevel::Owner);
        assert_eq!(cache.get_access_level(&client2, Some(user_id)), AccessLevel::Owner);
    }
    ...
    fn test_client_id_parameter_irrelevant_for_oauth_users() {
        ...
        cache.update_cache_for_user(&11111, Some(user_id), AccessLevel::Contributor, false);
        
        // Access level should be the same regardless of which client_id is used for lookup
        assert_eq!(cache.get_access_level(&11111, Some(user_id)), AccessLevel::Contributor);
        assert_eq!(cache.get_access_level(&99999, Some(user_id)), AccessLevel::Contributor);
    }
    ...
    fn test_pki_to_oauth_transition_preserves_higher_access() {
        ...
        cache.update_cache_for_user(&client_id, None, AccessLevel::Owner, false);
        ...
        cache.update_cache_for_user(&client_id, Some(user_id), AccessLevel::Viewer, false);
    ...
    fn test_get_access_level_method_routing() {
        ...
        cache.update_cache_for_user(&67890, Some(user_id), AccessLevel::Owner, false);
        
        // get_access_level should route correctly based on user_id presence
        assert_eq!(cache.get_access_level(&client_id, None), AccessLevel::Contributor);
        assert_eq!(cache.get_access_level(&67890, Some(user_id)), AccessLevel::Owner);
        assert_eq!(cache.get_access_level(&99999, Some(user_id)), AccessLevel::Owner); // User access, client_id irrelevant
        assert_eq!(cache.get_access_level(&99999, None), AccessLevel::None); // No PKI access for this client
    }
    ...
    fn test_pki_client_remove_access_with_override() {
        ...
        cache.update_cache_for_user(&client_id, None, AccessLevel::None, true);
        
        assert_eq!(cache.count(), 0);
        assert_eq!(cache.get_access_level_for_client(&client_id), AccessLevel::None);
        assert_eq!(cache.get_access_level(&client_id, None), AccessLevel::None);
    }
    ...
    fn test_pki_client_cannot_remove_access_without_override() {
        ...
        cache.update_cache_for_user(&client_id, None, AccessLevel::None, false);
        
        assert_eq!(cache.count(), 1);
        assert_eq!(cache.get_access_level_for_client(&client_id), AccessLevel::Owner);
        assert_eq!(cache.get_access_level(&client_id, None), AccessLevel::Owner);
    }
}