...
impl Shard {
    pub fn new(
        ...
        let aggregate_read_config = config.aggregate_read_config();
        let aggregate_write_config = config.aggregate_write_config();
        let node_config = config.node_config();

        let local_aggregate =
            LocalAggregate::new(aggregate_read_config, aggregate_write_config, node_config);
