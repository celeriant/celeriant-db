...
pub async fn write_message<W>(writer: &mut W, message: &Response, use_v2: bool) -> Result<(), WireError>
where
    W: AsyncWriteExt + Unpin,
{
    let protocol_version = if use_v2 { PROTOCOL_VERSION_V2 } else { PROTOCOL_VERSION_V1 };
...
async fn write_message_v2<W>(writer: &mut W, message: &Response, protocol_version: u32) -> Result<(), WireError>
where
    W: AsyncWriteExt + Unpin,
{
    match message {
        ...
        _ => {
            ...
            match bincode::encode_into_slice(message, &mut stack_buffer[8..], BINCODE_CONFIG_VARIABLE) {
                Ok(encoded_len) => {
...
async fn write_message_v1<W>(writer: &mut W, message: &Response, protocol_version: u32) -> Result<(), WireError>
where
    W: AsyncWriteExt + Unpin,
{
    match message {
    ...
    R: AsyncReadExt + Unpin,
{
    let message = if message_length <= STACK_BUFFER_SIZE as usize {
        let mut stack_buffer = [0u8; STACK_BUFFER_SIZE as usize];
        reader.read_exact(&mut stack_buffer[..message_length]).await?;
        bincode::decode_from_slice(&stack_buffer[..message_length], BINCODE_CONFIG_VARIABLE)
            .map_err(|_| WireError::InvalidFormat)?.0
    } else {
